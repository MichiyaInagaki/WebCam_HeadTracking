...
#define FACE_DOWNSAMPLE_RATIO 1		//デフォルト4
	...
	try
	{
...
#endif
		{
			...
			for (unsigned long i = 0; i < faces.size(); ++i)
			{
				...
				//string str = to_string(eav[1]) + "," + to_string(eav[0]);
				//string str = to_string(mod_yaw) + "," + to_string(eav[0]);
				string str = to_string(-mod_yaw) + "," + to_string(-mod_pitch+180) + "," + "e";
				const char* cstr = str.c_str();
				sendto(sock, cstr, 21, 0, (struct sockaddr*) & addr, sizeof(addr));
				closesocket(sock);
				WSACleanup();
				// End UDP

				printf("yaw: %.5f pitch: %.5f roll: %.5f\n", -mod_yaw, -mod_pitch + 180, eav[2]);
		...
		// Display it all on the screen
			...
			im_display = im;
			cv::resize(im, im_display, cv::Size(), 1.0, 1.0);
